...
char *create_response(
    ...
    if(encrypted == 1) {
        ...
        char *private_key = sha256_hash(otp);
        
        uint8_t buffer_key[44];
        hex_to_bytes(private_key, buffer_key, 32);
        uint8_t key[32];
        uint8_t nonce[12];
        ...
        write_log(" *** OTP : %s", otp);
        write_log(" *** Public Key : %s", public_key);
        write_log(" *** Private Key : %s", private_key);
        ...
        final_body_size = b64_len + strlen(separator) + strlen(public_key) + 1;
        ...
        strcat(final_body, separator);
        strcat(final_body, public_key);
